window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CN_PROVIDER = "workbuddy-cn";
    const GLOBAL_PROVIDER = "workbuddy-global";
    const PROVIDER_DETAILS = Object.freeze({
      [CN_PROVIDER]: { displayName: "WorkBuddy 中国区", siteName: "WorkBuddy 中国站" },
      [GLOBAL_PROVIDER]: { displayName: "WorkBuddy 国际版", siteName: "WorkBuddy 国际站" },
    });
    ...
    function workBuddyProviderId(value) {
      const raw = String(value ?? "");
      if (/WorkBuddy\s*国际版/i.test(raw)) return GLOBAL_PROVIDER;
      if (/WorkBuddy\s*中国区/i.test(raw)) return CN_PROVIDER;
      const normalized = normalizedProvider(value);
      if (!normalized || !WORKBUDDY_PROVIDER_PATTERN.test(normalized)) return undefined;
      return /(?:global|international)(?:-|$)/.test(normalized) ? GLOBAL_PROVIDER : CN_PROVIDER;
    }

    function isWorkBuddyProvider(value) {
      return workBuddyProviderId(value) !== undefined;
    }

    function providerDetails(value) {
      return PROVIDER_DETAILS[workBuddyProviderId(value) ?? CN_PROVIDER];
    }
    ...
    function workBuddyProviderForInput(input) {
      ...
      return workBuddyProviderId(provider) ?? workBuddyProviderId(editor.textContent);
    }

    function isWorkBuddy(input) {
      return workBuddyProviderForInput(input) !== undefined;
    }
    ...
    async function authRequest(path, body = {}, provider = CN_PROVIDER) {
      const options = {
        method: "POST",
        cache: "no-store",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ ...(body ?? {}), provider: workBuddyProviderId(provider) ?? CN_PROVIDER }),
      };
    ...
    function WorkBuddyCreditsDock({ useProjection }) {
      ...
      const providerId = workBuddyProviderId(provider);
      ...
      useEffect(() => {
        ...
        const load = async () => {
          ...
          let status;
          try {
            status = await authRequest("status", {}, providerId);
          } catch {
          ...
          try {
            const result = await authRequest("credits", { accountId: status.activeAccountId }, providerId);
...
`;
    ...
    const modelCatalogsByName = new Map();
    const modelCatalogFetchedAt = new Map();
    ...
    async function loadModelCatalog(providerValue, force = false) {
      const provider = workBuddyProviderId(providerValue) ?? CN_PROVIDER;
      const fetchedAt = modelCatalogFetchedAt.get(provider) ?? 0;
      if (!force && fetchedAt && Date.now() - fetchedAt < MODEL_CATALOG_TTL_MS) return modelCatalogs.get(provider) ?? [];
      if (modelCatalogPromises.has(provider)) return modelCatalogPromises.get(provider);
      modelCatalogFetchedAt.set(provider, Date.now());
      const promise = authRequest("models", {}, provider).then((result) => {
        ...
        modelCatalogs.set(provider, catalog);
        modelCatalogsByName.set(provider, byName);
        return catalog;
      }).catch(() => modelCatalogs.get(provider) ?? []).finally(() => {
        modelCatalogPromises.delete(provider);
      });
      modelCatalogPromises.set(provider, promise);
      return promise;
    }
    ...
    function modelForButton(button, providerValue) {
      const provider = workBuddyProviderId(providerValue ?? button.closest("section")?.dataset.workbuddyProvider) ?? CN_PROVIDER;
      const byName = modelCatalogsByName.get(provider) ?? new Map();
      const pinned = byName.get(normalizedModelName(button.dataset.workbuddyModel));
      return pinned ?? byName.get(normalizedModelName(button.title || button.textContent));
    }

    function enhanceModelButton(button, provider) {
      const model = modelForButton(button, provider);
      if (!model) return;
      ...
      if (button.dataset.workbuddyModelBound !== "true") {
        ...
        button.addEventListener("pointerenter", () => {
          const current = modelForButton(button, provider);
    ...
    function enhanceModelMetadata() {
      ...
      for (const section of document.querySelectorAll('section[role="group"][aria-labelledby]')) {
        ...
        const provider = Object.entries(PROVIDER_DETAILS).find(([, details]) => details.displayName === headingText)?.[0];
        if (!provider) continue;
        found = true;
        section.dataset.workbuddyModelGroup = "";
        section.dataset.workbuddyProvider = provider;
        ...
        for (const button of section.querySelectorAll('button, [role="menuitemradio"], [role="menuitem"]')) {
          if (modelForButton(button, provider)) enhanceModelButton(button, provider);
        }
        const fetchedAt = modelCatalogFetchedAt.get(provider) ?? 0;
        if (!modelCatalogPromises.has(provider) && (!fetchedAt || Date.now() - fetchedAt >= MODEL_CATALOG_TTL_MS)) {
          loadModelCatalog(provider).then(enhanceModelMetadata);
        }
      }
    ...
    function applyMode(input, keyButton, tokenButton, keySection, keySourceRow, keySelect, keyHint, newKeyInput, newKeyLabelInput, saveKeyButton, removeKeyButton, tokenSection, tokenHint, accountRow, accountSelect, accountName, addButton, removeButton, accountStats, message, status) {
      ...
      tokenHint.textContent = accounts.length
        ...
        : `在浏览器中完成${status.siteName || "WorkBuddy"}登录，返回后即可直接使用。`;
      ...
      message.textContent = token
        ? status.authenticated ? "账号令牌已安全保存" : `尚未登录${status.displayName || "WorkBuddy"}`
    ...
    async function request(path, body, provider) {
      return authRequest(path, body ?? {}, provider);
    }

    function mount(input, providerValue) {
      const provider = workBuddyProviderId(providerValue ?? workBuddyProviderForInput(input));
      if (!provider) return;
      const details = providerDetails(provider);
      const providerRequest = (path, body) => request(path, body, provider);
      ...
      controls.dataset.workbuddyProvider = provider;
      ...
      tokenHint.textContent = `${details.siteName}令牌登录后可切换账号，并查看积分与今日请求量。`;
      ...
      const loadCredits = async (accountId) => {
        ...
        try {
          const result = await providerRequest("credits", { accountId });
      ...
      keyButton.addEventListener("click", async () => {
        ...
        try {
          render(await providerRequest("api-key", current.activeApiKeyId ? { keyId: current.activeApiKeyId } : undefined));
          newKeyInput.focus();
        } catch (error) {
      ...
      keySelect.addEventListener("change", async () => {
        ...
        try {
          render(await providerRequest("api-key", { keyId: keySelect.value }));
        } catch (error) {
          message.textContent = error instanceof Error ? error.message : "切换失败";
          message.style.color = "var(--dsw-text-danger, #c62828)";
        } finally {
      ...
      saveKeyButton.addEventListener("click", async () => {
        ...
        try {
          ...
          const next = await providerRequest("api-key/add", { key: value, ...(label ? { label } : {}) });
          newKeyInput.value = "";
      ...
      removeKeyButton.addEventListener("click", async () => {
        ...
        try {
          render(await providerRequest("api-key/remove", { keyId: current.activeApiKeyId }));
        } catch (error) {
      ...
      tokenButton.addEventListener("click", async () => {
        ...
        message.textContent = current.accounts?.length ? "正在切换令牌账号…" : `请在浏览器中完成${details.siteName}登录`;
        try {
          const next = await providerRequest(current.accounts?.length ? "token" : "login");
          render(next);
      ...
      addButton.addEventListener("click", async () => {
        ...
        message.textContent = `请在浏览器中完成${details.siteName}登录`;
        try {
          const next = await providerRequest("login");
          render(next);
          await loadCredits(next.activeAccountId);
        } catch (error) {
          message.textContent = error instanceof Error ? error.message : "登录失败";
      ...
      accountSelect.addEventListener("change", async () => {
        ...
        try {
          const next = await providerRequest("token", { accountId: accountSelect.value });
          render(next);
          await loadCredits(next.activeAccountId);
        } catch (error) {
          message.textContent = error instanceof Error ? error.message : "切换失败";
      ...
      removeButton.addEventListener("click", async () => {
        ...
        try {
          const next = await providerRequest("remove", { accountId: current.activeAccountId });
          render(next);
          await loadCredits(next.activeAccountId);
        } catch (error) {
          message.textContent = error instanceof Error ? error.message : "删除失败";
    ...
    function WorkBuddyProviderCard({ provider }) {
      ...
      const providerId = workBuddyProviderId(provider?.provider);
      const supported = providerId !== undefined;
      useEffect(() => {
    ...
    function enhance() {
      for (const input of document.querySelectorAll('input[aria-label="API 密钥"]')) {
        const provider = workBuddyProviderForInput(input);
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        const resetCatalog = () => {
          modelCatalogs.clear();
          modelCatalogsByName.clear();
          modelCatalogFetchedAt.clear();
          enhance();
        };
